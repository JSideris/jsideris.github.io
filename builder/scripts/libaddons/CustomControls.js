...
THREE.CustomControls = function ( camera, domElement, scene, interactionPlane, eventCallbacks ) {
	...
	this.rotateCamera = ( function() {
		...
		return function rotateCamera() {
			...
			_this.camera.rotation.y -= angleY * _this.rotateSpeed;
			_this.camera.rotation.x = Math.max(-Math.PI/2, Math.min(Math.PI/2, _this.camera.rotation.x + angleX * _this.rotateSpeed));
	...
	this.updateMovementVector = function() {
		...
		if(_state == STATE.INTERACT && _this.mouselookMode){
			mouseX = (_dragTo.x - _dragStart.x) * 10;
			mouseY = (_dragTo.y - _dragStart.y) * 10;
			//console.log(mouseX);
		}

		_this.moveVector.x = ( - _this.moveState.left    + _this.moveState.right ) + mouseX;
		_this.moveVector.y = ( - _this.moveState.down    + _this.moveState.up ) + mouseY;
		_this.moveVector.z = ( - _this.moveState.forward + _this.moveState.back );
		//console.log(mouseX);
